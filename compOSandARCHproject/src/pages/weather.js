...
const Weather = () => {
	...
	const [locationArray, setLocationArray] = useState(['New York, NY']);
	...
	async function convertInitialAddressesToLatLng(providedAddress, i) {
		...
		let lng = '';
		let cityAndState = '';
		let formattedAddress = providedAddress.trim().replace(' ', ',+');
	...
	async function getAllWeather(address, i, initialValues) {
		try {
			...
			let currentMinutes = new Date().getMinutes().toLocaleString('en-US', {
			...
			let currentPrecipitationProbability = Math.round(
				response.data.data.timelines[currentIndex].intervals[0].values
					.precipitationProbability
			);
			let currentWindSpeed = Math.round(
	...
	async function getAllCityTimeZones(
		...
		try {
			...
			let apiESTAdjustment = response.data.rawOffset / 60 / 60;
			let localHour = Number(estHour) + Number(apiESTAdjustment);
